...
Credit: https://github.com/justinsalamon/melodia_python_tutorial/blob/master/melodia_python_tutorial.ipynb
...
class Melodia:
    ...
    useneg=False
    '''
    Code written by Justin Salamon
...
#         self.plot_melody(timestamps,melody,audio_file)
    ...
    def melodia_plugin_melosynth(self,audio_file,output_file,wav_file):
        # This is how we load audio using Essentia
        loader = es.MonoLoader(filename=audio_file, downmix = 'mix', sampleRate = 44100)
        audio = loader()
        # This is how we load audio using Librosa
...
#         print(sr)
        ...
        melody_pos = melody[:]
        melody_pos[melody<=0] = 0
        melody_df = [timestamps,melody_pos]
        df = pd.DataFrame(melody_df).T
        df.to_csv(output_file,index=False)


        self.melosynth(output_file,wav_file,self.fs,self.nHarmonics,self.square,self.useneg)
        return wav_file
    ...
    def wavwrite(self,x, filename, fs=44100, N=16):
        '''
    ...
    def loadmel(self,inputfile, delimiter=None):
        '''
    ...
    def melosynth_batch(self,inputfolder, outputfolder, fs, nHarmonics, square, useneg):
        '''
        ...
        for inputfile in inputfiles:
            ...
            self.melosynth(inputfile, outputfile, fs, nHarmonics, square, useneg)


    def melosynth(self,inputfile, outputfile, fs, nHarmonics, square, useneg):
        '''
        ...
        times, freqs = self.loadmel(inputfile)
        ...
        self.wavwrite(np.asarray(signal), outputfile, fs)
